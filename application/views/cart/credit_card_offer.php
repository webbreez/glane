...
$(document).ready(function(){
	$("#proceed_final").click(function(){
		...
		data.offer_id = "<?php echo $this->uri->segment(4)?>";

		$.ajax({
	        url: '<?php echo URL::site("cart/proceed_to_payment_offer");?>',
	        type: 'post',
	       	data: data,
	        dataType: 'json',
	        success: function(data) {
	        	///location.href="<?php echo URL::site('payment/process.php');?>?total="+data.total+"&uniqid="+data.uniqid;
	        	location.href="<?php echo URL::site('cart/order_processed_offer');?>/"+data.total+"/"+data.uniqid+"/"+data.offer_id;
	        }
	    });

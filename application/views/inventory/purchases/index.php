...
$(document).ready(function(){
	...
	$(".paid").live('click', (function () {
		var id = $(this).attr('id');
		$.post("<?php echo url::site()?>inventory/purchases/edit", {id: id, status: "Paid"}, function(data){
		 	location.href='<?php echo url::site()?>inventory/purchases';
	  	});
	}));

	$(".shipped").live('click', (function () {
		var id = $(this).attr('id');
		$.post("<?php echo url::site()?>inventory/purchases/edit", {id: id, status: "Shipped"}, function(data){
		 	location.href='<?php echo url::site()?>inventory/purchases';
	  	});
	}));
	...
	$(".received").live('click', (function () {
		var id = $(this).attr('id');
		$.post("<?php echo url::site()?>inventory/purchases/edit", {id: id, status: "Received"}, function(data){
		 	location.href='<?php echo url::site()?>inventory/purchases';
	  	});
	}));
	...
	oTable = $('#datatables').dataTable( {
		...
		"bProcessing": true,
		"bServerSide": true,
		"sAjaxSource": "<?php echo url::site()?>inventory/datatables/list_data/purchases/",
		"aaSorting": [[ 1, "desc" ]]
...
<table align="center" cellpadding="0" cellspacing="0" border="0" width="100%" class="table table-striped table-bordered" id="datatables">
	<thead>
		<tr>
			<th width="20%" id="payer_fname">Member Name</th>
			<th width="20%" id="invoice">Invoice</th>
			<th width="20%" id="product_name">Product Name</th>
			<th width="20%" id="payment_status">Status</th>
			<th width="20%">&nbsp;</th>
		</tr>
	</thead>
	<tbody>
